...
import com.cinescape1.databinding.ActivityPaymentBinding
...
import com.cinescape1.ui.main.views.payment.paymentList.adapter.PaymentAdapter
...
import com.cinescape1.ui.main.views.payment.paymentList.viewModel.PaymentViewModel
...
@ActivityScoped
class PaymentActivity : AppCompatActivity(), PaymentAdapter.RecycleViewItemClickListener {
...
//        )
    ...
    private fun movedNext() {
        ...
        binding?.txtProceed?.setOnClickListener {
            ...
            when (paymentViewModel.selectedPaymentMethod) {
                        ...
                        if (!Constant.IntentKey.TimerExtandCheck) {
                            ...
                            object : CountDownTimer((Constant.IntentKey.TimerExtand * 1000), 1000) {
                                ...
                                override fun onFinish() {
                                    val dialog = OptionDialog(this@PaymentActivity,
                                        R.mipmap.ic_launcher,
                                        R.string.app_name,
                        ...
                        if (VISA_PREFIX.contains(
                            ...
                            proceedAlertDialog.image_american_express_card.setImageDrawable(
                                ContextCompat.getDrawable(
                                    this@PaymentActivity, R.drawable.visa_card
                                )
                            )
                            ...
                            proceedAlertDialog.image_american_express_card.setImageDrawable(
                                ContextCompat.getDrawable(
                                    this@PaymentActivity, R.drawable.mastercard_card
                                )
                            )
                            ...
                            proceedAlertDialog.image_american_express_card.setImageDrawable(
                                ContextCompat.getDrawable(
                                    this@PaymentActivity, R.drawable.amerian_card
                                )
                            )
                ...
                try {
                    if (!proceedAlertDialog.cardNumberTextInputEditText.text.toString().isEmpty()) {
                        ...
                        if (VISA_PREFIX.contains(
                            ...
                            proceedAlertDialog.image_american_express_card.setImageDrawable(
                                ContextCompat.getDrawable(
                                    this@PaymentActivity, R.drawable.disover_card
                                )
                            )
                        } else if (JCB.contains(
                                proceedAlertDialog.cardNumberTextInputEditText.text.toString()
                            ...
                            proceedAlertDialog.image_american_express_card.setImageDrawable(
                                ContextCompat.getDrawable(
                                    this@PaymentActivity, R.drawable.jcb_card
                                )
                            )
                        } else if (MAESTRO.contains(
                                proceedAlertDialog.cardNumberTextInputEditText.text.toString()
                            ...
                            proceedAlertDialog.image_american_express_card.setImageDrawable(
                                ContextCompat.getDrawable(
                                    this@PaymentActivity, R.drawable.maestro_card
                                )
                            )
                            ...
                            proceedAlertDialog.image_american_express_card.setImageDrawable(
                                ContextCompat.getDrawable(
                                    this@PaymentActivity, R.drawable.uatp
                                )
                            )
                        } else {
                            proceedAlertDialog.image_american_express_card.visibility = View.GONE
    ...
    private fun creditCardInit(request: HmacKnetRequest) {
        paymentViewModel.creditCardInit(request).observe(this) {
            it?.let { resource ->
...
//
    ...
    private fun doProfile(sessions1: String, merchent: String) {
        ...
        val profilingHandle = TMXProfiling.getInstance().profile(
            options, PaymentListActivity.CompletionNotifier()
        )
    ...
    private fun postCardData(request: PostCardRequest) {
        paymentViewModel.postCardData(request).observe(this) {
            it?.let { resource ->
                when (resource.status) {
                    Status.SUCCESS -> {
                        resource.data?.let { it ->
    ...
    private fun validateJWT(s: ValidateJWTRequest) {
        paymentViewModel.validateJWT(s).observe(this) {
            it?.let { resource ->
                when (resource.status) {
                    Status.SUCCESS -> {
                        LoaderDialog.getInstance(R.string.pleasewait)?.dismiss()
    ...
    private fun ticketList(request: TicketSummaryRequest) {
        paymentViewModel.paymentList(request).observe(this) {
            it?.let { resource ->
                when (resource.status) {
                    Status.SUCCESS -> {
                        LoaderDialog.getInstance(R.string.pleasewait)?.dismiss()
    ...
    private fun retrieveData(output: PaymentListResponse.Output) {
        ...
        adapter = PaymentAdapter(this, output.payMode, this, paymentViewModel)
    ...
    private fun walletPay(request: HmacKnetRequest) {
        paymentViewModel.paymentWallet(request).observe(this) {
            it?.let { resource ->
                when (resource.status) {
                    Status.SUCCESS -> {
                        LoaderDialog.getInstance(R.string.pleasewait)?.dismiss()
    ...
    private fun paymentHmac(request: HmacKnetRequest) {
        paymentViewModel.paymentKnetHmac(request).observe(this) {
            it?.let { resource ->
                when (resource.status) {
                    Status.SUCCESS -> {
                        LoaderDialog.getInstance(R.string.pleasewait)?.dismiss()
...
//                            loader?.show(supportFragmentManager, null)
    ...
    private fun cancelTrans(cancelTransRequest: CancelTransRequest) {
        paymentViewModel.cancelTrans(cancelTransRequest).observe(this) {
            it?.let { resource ->
                when (resource.status) {
                    Status.SUCCESS -> {
                        LoaderDialog.getInstance(R.string.pleasewait)?.dismiss()
    ...
    private fun bankOfferApply(
        ...
        paymentViewModel.bankApply(bankOfferRequest).observe(this) {
            it?.let { resource ->
                when (resource.status) {
                    Status.SUCCESS -> {
                        LoaderDialog.getInstance(R.string.pleasewait)?.dismiss()
                        resource.data?.let { it ->
                            if (it.data?.result == Constant.status && it.data.code == Constant.SUCCESS_CODE) {
                                ...
                                paymentViewModel.setPaymentMethodSelection(PaymentMethodSealedClass.CREDIT_CARD)
                                bankCancel.show()
                                PaymentListActivity.bankApplied = true
                                PaymentListActivity.spinnerClickable = false
...
//                          //offer EditText
                                ...
                                paymentViewModel.setpaymentMethodSelectionStateFlow(
                                    PaymentMethodSealedClass.NONE
                                )
                                PaymentListActivity.bankApplied = false
                                PaymentListActivity.spinnerClickable = true
                                val dialog = OptionDialog(this,
                                    R.mipmap.ic_launcher,
                                    R.string.app_name,
                                    it.data?.msg.toString(),
    ...
    private fun bankOfferRemove(
        ...
        paymentViewModel.bankRemove(bankOfferRequest).observe(this) {
            it?.let { resource ->
                when (resource.status) {
                    Status.SUCCESS -> {
                        LoaderDialog.getInstance(R.string.pleasewait)?.dismiss()
                        ...
                        resource.data?.let { it ->
                            if (it.data?.result == Constant.status && it.data.code == Constant.SUCCESS_CODE) {
                                ...
                                outputlist?.clear()
                                outputlist?.addAll(it.data.output.payInfo)
                                paymentViewModel.setPaymentMethodSelection(PaymentMethodSealedClass.NONE)
                                //knet
...
//                          //offer
    ...
    private fun voucherApply(
        ...
        paymentViewModel.voucherApply(request).observe(this) {
            it?.let { resource ->
                when (resource.status) {
                    Status.SUCCESS -> {
                        LoaderDialog.getInstance(R.string.pleasewait)?.dismiss()
    ...
    private fun giftCardApply(
        ...
        paymentViewModel.giftCardApply(request).observe(this) {
            it?.let { resource ->
                when (resource.status) {
                    Status.SUCCESS -> {
                        LoaderDialog.getInstance(R.string.pleasewait)?.dismiss()
...
//                                    )
    ...
    private fun giftCardRemove(
        ...
        paymentViewModel.giftCardRemove(request).observe(this) {
            it?.let { resource ->
                when (resource.status) {
                    Status.SUCCESS -> {
                        LoaderDialog.getInstance(R.string.pleasewait)?.dismiss()
    ...
    private fun retrieveRemoveGiftCard(output: GiftCardRemove.Output, offerEditText: EditText, apply: TextView, imageCheck: ImageView, remove: ImageView) {
