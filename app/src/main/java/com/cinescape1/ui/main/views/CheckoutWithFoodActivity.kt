...
class CheckoutWithFoodActivity : DaggerAppCompatActivity() {
    ...
    private val checkoutWithFoodViewModel: CheckoutWithFoodViewModel by viewModels { viewModelFactory }
...
//            onBackPressed()
    ...
    private fun walletPay(request: HmacKnetRequest) {
        checkoutWithFoodViewModel.paymentWallet(request)
            .observe(this) {
                it?.let { resource ->
    ...
    private fun creditCardInit(request: HmacKnetRequest) {
        checkoutWithFoodViewModel.creditCardInit(request)
            .observe(this) {
                it?.let { resource ->
                    when (resource.status) {
                        Status.SUCCESS -> {
...
//                            loader?.dismiss()
    ...
    private fun validateJWT(s: ValidateJWTRequest) {
        checkoutWithFoodViewModel.validateJWT(s)
            .observe(this) {
                it?.let { resource ->
                    when (resource.status) {
                        Status.SUCCESS -> {
    ...
    private fun tckSummary(request: TicketSummaryRequest) {
        checkoutWithFoodViewModel.tckSummary(request)
            .observe(this) {
                it?.let { resource ->
                    when (resource.status) {
                        Status.SUCCESS -> {
    ...
    private fun paymentHmac(request: HmacKnetRequest) {
        checkoutWithFoodViewModel.paymentKnetHmac(request)
            .observe(this) {
                it?.let { resource ->
                    when (resource.status) {
                        Status.SUCCESS -> {
...
//            val layoutManager = LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL,false)
    ...
    private fun cancelTrans(cancelTransRequest: CancelTransRequest) {
        checkoutWithFoodViewModel.cancelTrans(cancelTransRequest)
            .observe(this) {
                it?.let { resource ->
                    when (resource.status) {
                        Status.SUCCESS -> {
