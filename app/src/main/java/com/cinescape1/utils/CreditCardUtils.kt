...
object CreditCardUtils {
    ...
    const val JCB_PREFIX = "3"
    const val MAESTRO_PREFIX = "50"
    fun getCardType(cardNumber: String): Int {
        ...
        ) return AMEX else if (cardNumber.substring(
                0,
                4
            ) == DISCOVER_PREFIX
        ) return DISCOVER else if (cardNumber.substring(
                0,
                1
            ) == JCB_PREFIX
        ) return JCB else if (cardNumber.substring(0, 2) == MAESTRO_PREFIX) return MAESTRO
        return NONE
    }

    fun isValid(cardNumber: String): Boolean {
        return if (!TextUtils.isEmpty(cardNumber) && cardNumber.length >= 4) if (getCardType(
                cardNumber
            ) == VISA && (cardNumber.length == 13 || cardNumber.length == 16)
        ) true else if (getCardType(cardNumber) == MASTERCARD && cardNumber.length == 16) true else if (getCardType(
                cardNumber
            ) == AMEX && cardNumber.length == 15
        ) true else if (getCardType(cardNumber) == DISCOVER && cardNumber.length == 16) true else if (getCardType(
                cardNumber
            ) == JCB && cardNumber.length == 16
        ) true else getCardType(cardNumber) == MAESTRO && cardNumber.length == 16 else false
    }
